...
export default function Review({ unit, checklist, responses, meta, setMeta, customerSigned, setCustomerSigned, inspectorSigned, setInspectorSigned, onBack, onDeleteIssue, onGenerate }) {
  ...
  return (
    <>
      <div style={{ padding: '14px 18px', background: '#fff', borderBottom: '1px solid #EDEFF2', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div onClick={onBack} style={{ width: 36, height: 36, borderRadius: 10, border: '1px solid #E6E8EC', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
          <i className="ph ph-arrow-left" style={{ fontSize: 18 }} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#6B7A99' }}>Review</div>
      ...
      <div className="scrl" style={{ flex: 1, overflow: 'auto', padding: '16px 16px 120px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #EDEFF2', borderRadius: 16, padding: 18, marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: '#141B2D' }}>Total {totalIssues} issues</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {rooms.map((r) => (
              <div key={r.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: '1px solid #F4F5F7' }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: stats[r.key].issues > 0 ? '#D9600A' : '#B7BEC9' }}>{stats[r.key].issues}</span>
              </div>
            ))}
          </div>
        </div>

        <div style={{ fontSize: 12, fontWeight: 700, color: '#6B7A99', letterSpacing: '.02em', margin: '6px 4px 10px' }}>INSPECTION DETAILS</div>
        <div style={{ background: '#fff', border: '1px solid #EDEFF2', borderRadius: 16, padding: 16, marginBottom: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {META_FIELDS.map((f) => (
            <div key={f.key}>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#8A94A6', display: 'block', marginBottom: 5 }}>{f.label}</label>
              <input type={f.type} value={meta[f.key] || ''} placeholder={f.placeholder} onChange={(e) => setMeta((m) => ({ ...m, [f.key]: e.target.value }))}
                style={{ width: '100%', height: 44, border: '1px solid #E6E8EC', borderRadius: 10, padding: '0 12px', fontSize: 14, fontWeight: 500, color: '#141B2D', outline: 0 }} />
            </div>
          ))}
        </div>

        <div style={{ fontSize: 12, fontWeight: 700, color: '#6B7A99', letterSpacing: '.02em', margin: '6px 4px 10px' }}>ALL ISSUES ({totalIssues})</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 14 }}>
          {allIssues.map((ai, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#fff', border: '1px solid #EDEFF2', borderRadius: 14, padding: '11px 13px' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#6B7A99', marginBottom: 1 }}>{ai.crumb}</div>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#141B2D', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ai.heading}</div>
                <div style={{ fontSize: 11, color: '#8A94A6', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ai.room} · {ai.exact_location}</div>
              </div>
        ...
        {skippedItems.length > 0 && (
          <div style={{ background: '#F7F8FA', border: '1px solid #EDEFF2', borderRadius: 14, padding: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#6B7A99', marginBottom: 8 }}>SKIPPED ({skippedItems.length})</div>
            {skippedItems.map((sk, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 12, color: '#6B7A99' }}>
                <i className="ph ph-arrow-right" style={{ color: '#8A94A6' }} /><span style={{ flex: 1 }}>{sk.text}</span><span style={{ color: '#B7BEC9' }}>{sk.room}</span>
              </div>
            ))}
          </div>
        )}

        <div style={{ fontSize: 12, fontWeight: 700, color: '#6B7A99', letterSpacing: '.02em', margin: '6px 4px 10px' }}>PARTICIPANT SIGN-OFF</div>
        <div style={{ display: 'flex', gap: 10 }}>
          {signatures.map((sg) => (
            <div key={sg.role} onClick={sg.onSign}
              style={{ flex: 1, border: `1.5px solid ${sg.signed ? '#EA4E20' : '#E6E8EC'}`, borderRadius: 14, padding: 12, cursor: 'pointer', background: sg.signed ? '#FEF6F3' : '#fff' }}>
              <div style={{ height: 40, borderBottom: '1.5px dashed #D4D9E0', display: 'flex', alignItems: 'flex-end', paddingBottom: 3 }}>
                <span style={sg.signed ? { fontFamily: "'Segoe Script','Bradley Hand',cursive", fontSize: 19, color: '#141B2D' } : { fontSize: 12, color: '#B7BEC9', fontWeight: 600 }}>
                  {sg.signed ? sg.name.split(' ')[0] : 'Tap to sign'}
                </span>
              </div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#8A94A6', marginTop: 7 }}>{sg.role}</div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#141B2D' }}>{sg.name}</div>
            </div>
          ))}
        </div>
      </div>
