...
export default function Safety({ safety, setSafety, onBack, onContinue }) {
  ...
  return (
    <div className="scrl" style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', padding: 24, animation: 'fade .25s ease' }}>
      <div onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#667085', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 22 }}>
      ...
      <div style={{ fontSize: 14, color: '#667085', lineHeight: 1.5, marginBottom: 24 }}>This is an active handover site. Confirm the following before check-in.</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {ITEMS.map((s) => {
          const on = safety[s.key]
          return (
            <div key={s.key} onClick={() => setSafety((x) => ({ ...x, [s.key]: !x[s.key] }))}
              style={{ display: 'flex', alignItems: 'center', gap: 13, padding: 15, borderRadius: 14, cursor: 'pointer', border: `1.5px solid ${on ? '#F04E38' : '#EAECF0'}`, background: on ? '#FEF2ED' : '#fff' }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: on ? '#F04E38' : '#667085', background: on ? '#FDEBD9' : '#F1F2F4' }}>
                <i className={s.icon} style={{ fontSize: 16 }} />
              </div>
              <span style={{ flex: 1, fontSize: 15, fontWeight: 600, color: '#0D0D0D' }}>{s.label}</span>
              <i className={on ? 'ph-fill ph-check-circle' : 'ph ph-circle'} style={{ fontSize: 22, color: on ? '#F04E38' : '#D4D9E0' }} />
            </div>
          )
        })}
      </div>
      <div style={{ flex: 1 }} />
      <button className="btn-primary" style={{ marginTop: 20, background: ok ? '#F04E38' : '#F0D2C6', cursor: ok ? 'pointer' : 'not-allowed' }} onClick={onContinue}>
