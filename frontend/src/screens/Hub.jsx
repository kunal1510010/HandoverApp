...
export default function Hub({ unit, checklist, responses, offline, onOpenRoom, onReview }) {
  ...
  return (
    <>
      <div style={{ padding: '18px 20px 14px', background: '#fff', borderBottom: '1px solid #EDEFF2', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#6B7A99' }}>{unit.project} · {unit.config}</div>
            <div style={{ fontSize: 21, fontWeight: 700, letterSpacing: '-.02em' }}>Unit {unit.unit_no}</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: 11, border: '1px solid #E6E8EC', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Connectivity">
            <i className={offline ? 'ph ph-cloud-slash' : 'ph ph-wifi-high'} style={{ fontSize: 19, color: '#6B7A99' }} />
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 7 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: '#141B2D', flexShrink: 0 }}>Overall progress</span>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#EA4E20', whiteSpace: 'nowrap', flexShrink: 0 }}>{overallPct}% · {totalDone}/{totalItems}</span>
        </div>
        <div style={{ height: 9, borderRadius: 9999, background: '#EEF0F3', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${overallPct}%`, background: '#EA4E20', borderRadius: 9999, transition: 'width .3s ease' }} />
        </div>
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          <div style={{ flex: 1, background: '#FDEBD9', borderRadius: 12, padding: '10px 12px' }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#D9600A', lineHeight: 1 }}>{totalIssues}</div>
          ...
          <div style={{ flex: 1, background: '#EEF0F3', borderRadius: 12, padding: '10px 12px' }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#141B2D', lineHeight: 1 }}>{roomsLeft}</div>
            <div style={{ fontSize: 11, color: '#6B7A99', fontWeight: 600, marginTop: 2 }}>Rooms left</div>
          </div>
        </div>
      </div>
        ...
        {rooms.map((r) => {
          ...
          return (
            <div key={r.key} onClick={() => onOpenRoom(r.key)}
              style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: '1px solid #EDEFF2', borderRadius: 16, padding: '13px 14px', marginBottom: 9, cursor: 'pointer' }}>
              <div style={{ width: 46, height: 46, borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: s.issues > 0 ? '#D9600A' : '#5A6478', background: s.issues > 0 ? '#FDEBD9' : '#EEF2F7' }}>
                <i className={ROOM_ICON[r.type] || 'ph ph-cube'} style={{ fontSize: 22 }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: '#141B2D' }}>{r.label}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
                  <span style={{ fontSize: 12, color: '#6B7A99' }}>{s.done}/{s.total} checked</span>
                  {s.issues > 0 && (
                    <span style={{ fontSize: 11, fontWeight: 600, color: '#D9600A', background: '#FDEBD9', padding: '2px 8px', borderRadius: 9999 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '14px 16px', background: 'linear-gradient(to top,#F5F6F8 70%,rgba(245,246,248,0))', display: 'flex', gap: 10 }}>
        ...
        <button onClick={onReview} title="Review & Generate"
          style={{ width: 56, height: 52, border: '1px solid #E6E8EC', borderRadius: 14, background: '#fff', color: '#141B2D', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
