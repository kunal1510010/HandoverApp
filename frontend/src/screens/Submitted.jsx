  ...
  return (
    <div className="scrl" style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', padding: '24px 22px 22px', animation: 'fade .25s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <span style={{ fontSize: 16, fontWeight: 800, color: '#EA4E20' }}>ASBL</span>
        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.14em', color: '#6B7A99' }}>SPECTRA</span>
      </div>
      <div style={{ width: 72, height: 72, borderRadius: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', color: issued ? '#D9600A' : '#12805A', background: issued ? '#FDEBD9' : '#EAF7F1' }}>
      ...
      <div style={{ fontSize: 14, color: '#6B7A99', lineHeight: 1.5, marginBottom: 22 }}>
        {issued
      ...
      <div style={{ background: '#fff', border: '1px solid #EDEFF2', borderRadius: 18, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '16px 18px', borderBottom: '1px solid #F2F3F5' }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: '#141B2D' }}>{unit.unit_no}</div>
            <div style={{ fontSize: 12, color: '#6B7A99' }}>{unit.config} · {unit.customer_name}</div>
          </div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 9999, whiteSpace: 'nowrap', color: issued ? '#D9600A' : '#12805A', background: issued ? '#FDEBD9' : '#EAF7F1' }}>
        ...
        <div style={{ padding: '6px 18px 10px' }}>
          {summary.map((s) => (
            <div key={s.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 0', borderBottom: '1px solid #F4F5F7' }}>
              <span style={{ fontSize: 13, color: '#6B7A99' }}>{s.label}</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#141B2D' }}>{s.value}</span>
            </div>
          ))}
        </div>
      </div>
