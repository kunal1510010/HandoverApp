      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 21, background: '#fff', borderRadius: '22px 22px 0 0', maxHeight: '92%', display: 'flex', flexDirection: 'column', animation: 'sheetUp .28s cubic-bezier(.2,.8,.2,1)' }}>
        <div style={{ padding: '14px 20px 6px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <i onClick={onClose} className="ph ph-x" style={{ fontSize: 22, color: '#667085', cursor: 'pointer' }} />
          </div>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#667085', marginTop: 2 }}>{draft.category} → {draft.subcategory} · {draft.subLocation}</div>
        </div>
        <div className="scrl" style={{ flex: 1, overflow: 'auto', padding: '6px 20px 16px' }}>
          <div style={{ fontSize: 12, color: '#667085', background: '#F7F8FA', borderRadius: 10, padding: '10px 12px', lineHeight: 1.4, marginBottom: 16 }}>{draft.checklistText}</div>

          <label style={{ fontSize: 12, fontWeight: 700, color: '#0D0D0D', display: 'block', marginBottom: 8 }}>Photos <span style={{ color: '#C4320A' }}>*</span></label>
          <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap', marginBottom: 6 }}>
            <div onClick={() => !uploading && fileInput.current?.click()}
              style={{ width: 78, height: 78, borderRadius: 14, background: '#F04E38', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, cursor: uploading ? 'wait' : 'pointer', flexShrink: 0, opacity: uploading ? 0.7 : 1 }}>
            ...
            {draft.photos.map((p, i) => (
              <div key={p.id} style={{ position: 'relative', width: 78, height: 78, borderRadius: 14, flexShrink: 0, overflow: 'hidden' }}>
                ...
                <i onClick={() => onRemovePhoto(i)} className="ph-fill ph-x-circle"
                  style={{ position: 'absolute', top: -6, right: -6, fontSize: 20, color: '#0D0D0D', background: '#fff', borderRadius: 9999, cursor: 'pointer' }} />
              </div>
            ))}
          </div>
          <div style={{ fontSize: 11, color: '#667085', marginBottom: 18 }}>At least one photo is required.</div>

          <label style={{ fontSize: 12, fontWeight: 700, color: '#0D0D0D', display: 'block', marginBottom: 7 }}>Heading</label>
          <input value={draft.heading} onChange={(e) => onHeadingChange(e.target.value)}
            style={{ width: '100%', height: 48, border: '1px solid #EAECF0', borderRadius: 11, padding: '0 13px', fontSize: 15, fontWeight: 500, color: '#0D0D0D', outline: 0, marginBottom: 16 }} />

          <label style={{ fontSize: 12, fontWeight: 700, color: '#0D0D0D', display: 'block', marginBottom: 7 }}>Exact location <span style={{ color: '#C4320A' }}>*</span></label>
          <textarea value={draft.location} onChange={(e) => onLocationChange(e.target.value)} rows={2} placeholder="e.g. 6th row of the 1st tile, near the balcony door"
            style={{ width: '100%', border: '1px solid #EAECF0', borderRadius: 11, padding: '12px 13px', fontSize: 14, color: '#0D0D0D', outline: 0, resize: 'none', lineHeight: 1.4 }} />
        ...
        <div style={{ padding: '12px 20px 18px', borderTop: '1px solid #EAECF0', display: 'flex', flexDirection: 'column', gap: 9, flexShrink: 0 }}>
